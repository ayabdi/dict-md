...
const App : any= ({ classes }) => {
  ...
  function handleTranscribeTimeoutChange(newTimeout: React.SetStateAction<number>) {
  ...
  function transcribeRecording(recordedBlob: { blob: string | Blob; }) {
    ...
    axios.post("http://127.0.0.1:8000/transcribe", formData, { headers })
      .then((res) => {
  ...
  return (
    <div className={classes.root}>
      ...
      <div>
        ...
        {transcribedData}
        <PulseLoader  size={20} color="purple" loading={isTranscribing} />
      </div>
    </div>
  );
}

const Apps : any = withStyles(useStyles)(App);
function render() {
  ReactDOM.render(<div><Apps/></div>, document.body);
}
